...
if [ -z "${INPUT_SIGNIFY_SEC_KEY_PASS}" ]; then
  signify-openbsd -S -s ${KEY_PATH} -m ${SHA256_MSG} -x ${RELEASE_ASSET_NAME}${RELEASE_ASSET_EXT}${SIGNIFY_EXT}
else
  echo "${INPUT_SIGNIFY_SEC_KEY_PASS}" | signify-openbsd -S -s ${KEY_PATH} -m ${SHA256_MSG} -x ${RELEASE_ASSET_NAME}${RELEASE_ASSET_EXT}${SIGNIFY_EXT}
fi
