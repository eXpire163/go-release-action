...
if [[ "${INPUT_BUILD_COMMAND}" =~ ^make.* ]]; then
    ...
    GOOS=${INPUT_GOOS} GOARCH=${INPUT_GOARCH} ${INPUT_BUILD_COMMAND} -o ${BUILD_ARTIFACTS_FOLDER}/${BINARY_NAME}${EXT} ${INPUT_BUILD_FLAGS} ${LDFLAGS_PREFIX} "${INPUT_LDFLAGS}" ${INPUT_SOURCE_FILES}
fi
